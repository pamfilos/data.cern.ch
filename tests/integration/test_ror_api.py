...
def test_ror_by_query(mock_ror, app, auth_headers_for_superuser, json_headers):
    ...
    mock_ror.return_value = {
        'items': [{'acronyms': ['TiU'], 'aliases': [], 'id': 'https://ror.org/04b8v1s79',
                   ...
                   'external_ids': {'FundRef': {'all': ['501100007659'], 'preferred': None},
                                    'GRID': {'all': 'grid.12295.3d', 'preferred': 'grid.12295.3d'},
                                    'ISNI': {'all': ['0000 0001 0943 3265'], 'preferred': None},
                                    'OrgRef': {'all': ['1070237'], 'preferred': None},
                                    'Wikidata': {'all': ['Q595668'], 'preferred': None}},
                   'labels': [{'iso639': 'nl', 'label': 'Universiteit van Tilburg'}],
                   'links': ['https://www.tilburguniversity.edu/'],
                   'name': 'Tilburg University', 'types': ['Education'],
                   'wikipedia_url': 'http://en.wikipedia.org/wiki/Tilburg_University'}],
        'meta': {'countries': [{'count': 1, 'id': 'nl', 'title': 'Kingdom of the Netherlands'}],
                 'types': [{'count': 1, 'id': 'education', 'title': 'Education'}]},
...
def test_ror_by_query_empty_query(mock_ror, app, auth_headers_for_superuser, json_headers):

    with app.test_client() as client:
        resp = client.get('services/ror', data=json.dumps({}),
                          headers=auth_headers_for_superuser + json_headers)
        assert resp.status_code == 200
...
def test_ror_by_org(mock_ror, app, auth_headers_for_superuser, json_headers):
    ...
    mock_ror.return_value = {
        'acronyms': ['TiU'], 'aliases': [],
        'country': {'country_code': 'NL', 'country_name': 'Netherlands'},
        'external_ids': {'FundRef': {'all': ['501100007659'], 'preferred': None},
                         'GRID': {'all': 'grid.12295.3d', 'preferred': None},
                         'ISNI': {'all': ['0000 0001 0943 3265'], 'preferred': None},
                         'OrgRef': {'all': ['1070237'], 'preferred': None},
                         'Wikidata': {'all': ['Q595668'], 'preferred': None}},
        'id': 'https://ror.org/04b8v1s79',  'name': 'Tilburg University',
        'labels': [{'iso639': 'nl', 'label': 'Universiteit van Tilburg'}],
        'links': ['https://www.tilburguniversity.edu/'],
        'types': ['Education'], 'wikipedia_url': 'http://en.wikipedia.org/wiki/Tilburg_University'}, 200
    ...
    with app.test_client() as client:
        ...
        assert resp.status_code == 200
        assert resp.json == {"acronyms": ["TiU"], "aliases": [], "types": ["Education"],
                             "country": {"country_code": "NL", "country_name": "Netherlands"},
                             "external_ids": {"FundRef": "501100007659", "GRID": "grid.12295.3d",
...
@patch('cap.modules.services.views.ror._ror', side_effect=ExternalAPIException())
def test_ror_by_query_exception(mock_ror, app, auth_headers_for_superuser, json_headers):
    mock_ror_id = '04b8v1s79'
...
@patch('cap.modules.services.views.ror._ror', side_effect=JSONDecodeError())
def test_ror_by_query_empty_list_exception(mock_ror, app, auth_headers_for_superuser, json_headers):
    mock_ror_id = '11111'
